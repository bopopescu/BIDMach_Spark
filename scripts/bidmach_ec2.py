...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key, copyfiles):
    ...
    subordinate_names = [get_dns_name(i, opts.private_ips) for i in subordinate_nodes];
    subordinates_string = reduce(lambda x,y : x + "\n" + y, subordinate_names);
    
    hscommand="echo -e '%s' > %s/conf/subordinates" % (subordinates_string, spark_install_dir)
    ssh(main, opts, hscommand.encode('ascii','ignore'))

    sscommand="echo -e '%s' > %s/etc/hadoop/subordinates" % (subordinates_string, hadoop_install_dir)

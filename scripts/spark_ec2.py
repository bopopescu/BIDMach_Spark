...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key, copyfiles):
    ...
    tmpuser = opts.user
    opts.user = "ec2-user"
    ssh(main, opts, """sudo cp -f /home/ec2-user/.ssh/authorized_keys /root/.ssh""")
    opts.user = tmpuser
    if deploy_ssh_key:
        print("Generating cluster's SSH key on main...")
        ssh(main, opts, """rm -f /root/.ssh/id_rsa""")
        key_setup = """
            (ssh-keygen -q -t rsa -N '' -f ~/.ssh/id_rsa &&
             cat ~/.ssh/id_rsa.pub >> ~/.ssh/authorized_keys)
        """
        ssh(main, opts, key_setup)
        ssh(main, opts, """rm -f /root/.ssh/known_hosts""")
        ...
        for subordinate in subordinate_nodes:
            ...
            tmpuser = opts.user
            opts.user = "ec2-user"
            ssh(subordinate_address, opts, """sudo cp -f /home/ec2-user/.ssh/authorized_keys /root/.ssh""")
            opts.user = tmpuser
            ssh_write(subordinate_address, opts, ['tar', 'x'], dot_ssh_tar)
            ssh(subordinate_address, opts, """rm -f /root/.ssh/known_hosts""")
    ...
    print("Deploying config files to main...")
    deploy_files(
        conn=conn,
    ...
    setup_spark_cluster(main, opts, copyfiles)
    print("Done!")


def setup_spark_cluster(main, opts, copyfiles):
...
def real_main():
    ...
    if action == "launch":
        ...
        setup_cluster(conn, main_nodes, subordinate_nodes, opts, True, False)
